...
H_mean_FilenName=sprintf('%s/Health_affine_restore_Gradient_mean.nii.gz',H_DIR);
% [0 177]


H_mean_nii=load_untouch_nii(H_mean_FilenName); H_mean=H_mean_nii.img;
H_std_FilenName=sprintf('%s/Health_affine_restore_Gradient_std.nii.gz',H_DIR);
...
for SubIdx=2:1:10
    filename=sprintf('%s/Patient_%03d/6_Features1/Patient_%03d_affine_restore_Gradient.nii.gz',DIR,SubIdx,SubIdx);
    ...
    OutName=sprintf('%s/Patient_%03d/Hong2014/z_score/Patient_%03d_Gradient_z_score.nii.gz',DIR,SubIdx,SubIdx);

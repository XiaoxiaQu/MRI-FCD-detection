...
int main( int argc, char* argv[] )
{
    ...
    unsigned int h= 100; // iteration times
    if (argc > 3)
    {
        h = atoi(argv[3]);
    }
    PixelType  GM=100; // intensity in the region of interest (ROI)
    //*****************Parametrs***************
    ...
    ImageType::Pointer     InImage  = reader1->GetOutput();
    ImageType::Pointer    OutImage = reader2->GetOutput();
    ...
    ImageRegionIteratorWithIndexType  in (InImage, region);
    ...
    NeighborhoodIteratorType out (radius,OutImage, region);

    for (unsigned int i=1; i<h; i++) //
    {
        for ( in.GoToBegin(), out.GoToBegin(); !in.IsAtEnd(); ++out, ++in)
        {
            if (in.Get()==GM) // make sure this pixel is in the ROI
            {
                float Mean;
